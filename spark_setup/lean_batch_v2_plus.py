...
# mapping function for the database
def map_merge_top(x):
    def merge_list(index, offset):
        d = dict(x[index])
        # loop through and put in dictionary
        for val in x[index + offset]:
            if val[0] in d:
                count = d.get(val[0])
                d.update({val[0]: count + val[1]})
            else:
    ...
    uid = x[0]
    user_name = x[1]
    rt_word_list = merge_list(2, 10)
    rt_entity_list = merge_list(3, 10)
    fav_word_list = merge_list(4, 10)
    fav_entity_list = merge_list(5, 10)
    rt_word_occur_list = merge_list(6, 10)
    rt_entity_occur_list = merge_list(7, 10)
    fav_word_occur_list = merge_list(8, 10)
    fav_entity_occur_list = merge_list(9, 10)

    return (uid, user_name,
            rt_word_list, rt_entity_list, fav_word_list, fav_entity_list,
            rt_word_occur_list, rt_entity_occur_list,
            fav_word_occur_list, fav_entity_occur_list
            )

# --> exec_path_for_all time,
# Input: rez_dic_set_rdd
# Output: all time --> save to database, && update user_top_list
def exec_path_for_alltime_v2(rez_dic_set_rdd):
    all_time_rdd = compute_total_count(rez_dic_set_rdd)
    rez = all_time_rdd.mapValues(get_top_n_alltime)
    # transform to the format for dataframe
    transformed_rdd = rez.map(transform_alltime_rdd)
    df = to_alltime_df(transformed_rdd)
    # read from database t1 and merge with current
    t1 = spark.read \
        .format("org.apache.spark.sql.cassandra") \
        .options(table="user_top_list", keyspace="twitter") \
        .load()
    # combined rez
    rez = t1.join(df, t1.user_name == df.user_name, 'inner')
    rdd_final = rez.rdd.map(map_merge_top)
    # convert back to dataframe
    df_final = to_alltime_df(rdd_final)
    save_data_frame(df_final, "user_top_list")
    return df_final
...
df_alltime = exec_path_for_alltime_v2(rez_dic_set_rdd)
